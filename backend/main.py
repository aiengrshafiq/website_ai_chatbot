...
# This sets up a simple logger to output to the console (which Azure will capture)
...
# This class encapsulates all Pipedrive logic, making the code cleaner and more organized.
class PipedriveManager:
    ...
    async def _find_person(self, field: str, value: str) -> int | None:
        """Finds a person by a specific field (email or phone)."""
    ...
    async def get_or_create_person_and_deal(self, name: str, email: str, phone: str) -> str:
        """Main logic to find/create a person and then create a deal."""
        ...
        try:
            ...
            # Step 3: Create a Deal linked to the person.
            async with httpx.AsyncClient() as http_client:
...
async def chat(request: Request):
    # We now get the raw body to pass to Pydantic for validation
...
"""
    ...
    tools = [
        {
            ...
            "function": {
                ...
                "description": "Create a new deal in the Pipedrive CRM when you have the user's full name, email, and phone number.",
                "parameters": {
                    "type": "object",
                    "properties": {
    ...
    try:
        ...
        # Fallback to streaming if no tool call
